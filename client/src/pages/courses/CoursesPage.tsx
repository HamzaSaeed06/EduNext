    ...
    <AppShell>
      ...
      >
        <div>
          ...
          <p className="text-body text-ink-muted">{total > 0 ? `${total} trail${total === 1 ? '' : 's'} available` : 'Browse trails across every discipline'}</p>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="h-44 bg-bg-surface-alt rounded-card animate-pulse" />
            ))}
          </div>
        ) : courses.length === 0 ? (
          <Card className="p-10 text-center">
            <p className="text-body text-ink-muted">No courses match your filters.</p>
          </Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
