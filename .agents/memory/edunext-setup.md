...
- `NODE_ENV` is NOT set by default by the `npm run dev` nodemon command — the log message uses `|| 'development'` for display only. The CORS bypass `process.env.NODE_ENV === 'development'` was silently never firing.
- **Fix applied:** set `NODE_ENV=development` explicitly in the `dev` script in `server/package.json` (`"dev": "NODE_ENV=development nodemon src/app.js"`).
- **How to apply going forward:** always prefix dev scripts with `NODE_ENV=development` rather than relying on the Node.js default. Also build the CORS allowlist by merging localhost defaults + any configured `CORS_ORIGIN` + an auto-derived entry from `process.env.REPLIT_DEV_DOMAIN` (`https://${REPLIT_DEV_DOMAIN}` and `:5000` variant).
